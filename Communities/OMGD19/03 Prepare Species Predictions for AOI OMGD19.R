...
kgrid_subset <- kgrid |>
  ...
  select(-NrName) |>
  st_as_sf(coords = c("X", "Y"), crs = st_crs(aoi_omgd19))

# Spatial subset for the grid cells in the OMGD19 AOI
kgrid_aoi <- kgrid_subset[st_intersects(kgrid_subset, aoi_omgd19, sparse = FALSE), ]
