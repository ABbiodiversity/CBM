...
# Title:   Prepare ABMI species predictions for BLCN AOI
# Date:    March 2026
...
# Species
species <- c("White-tailed Deer", "Black Bear", "Moose",
...
# Area of Interest (AOI) for BLCN - Prepared by D. Evans
aoi_blcn <- st_read(paste0(g_drive_cbme, "BLCN/AOI/BLCN_Report_AOI.shp")) |>
  st_transform(3400)
...
kgrid_subset <- kgrid |>
  ...
  select(-NrName) |>
  st_as_sf(coords = c("X", "Y"), crs = st_crs(aoi_blcn))

# Spatial subset for the grid cells in the BLMS AOI
kgrid_aoi <- kgrid_subset[st_intersects(kgrid_subset, aoi_blcn, sparse = FALSE), ]
...
for (sp in species) {
  ...
  # Save raster
  terra::writeRaster(r,
                     filename = paste0(g_drive_cbme, "BLCN/Predictions/", sp, " BLCN.tif"),
                     overwrite = TRUE)
