# ----------------------------------------------------------
# title:   Process WLFN camera data from WildTrax
#
...
# 2.0 Download WLFN data

# WLFN projects
wlfn_proj <- wt_get_projects(sensor = "CAM") |>
  filter(project_id == "3864") |>
  select(project, project_id)

# WLFN project IDs
wlfn_proj_ids <- wlfn_proj$project_id

# Download main and image reports (as list object)
ls_wlfn_reports <- map(.x = wlfn_proj_ids,
...
df_image_reports <-ls_wlfn_reports |>
  # Flatten nested list
  list_flatten() |>
  ...
  list_rbind() |>
  # Obtain `project` name
  left_join(wlfn_proj, by = "project_id") |>
...
df_main_reports <-ls_wlfn_reports |>
  # Flatten nested list
  list_flatten() |>
...
# Location reports
df_location_reports <- ls_wlfn_reports |>
  list_flatten() |>
  # Keep only main reports
...
sheet_id <- drive_find(type = "spreadsheet",
  ...
  filter(str_detect(name, "WLFN")) |>
  select(id) |>
  pull()

df_wlfn_edd <- read_sheet(ss = sheet_id) |>
...
save(df_density_sum, # Densities by location and species
     ...
     file = paste0(g_drive_cbme, "WFL128/Data/WFL128 Data Objects.RData"))
