...
aoi_wlfn <- st_read(paste0(g_drive_cbme, "WFL128/AOI/WLFN_Camera_AOI.shp")) |>
  st_transform(3400)
...
kgrid_subset <- kgrid |>
  ...
  select(-NrName) |>
  st_as_sf(coords = c("X", "Y"), crs = st_crs(aoi_wlfn))

# Spatial subset for the grid cells in the BLMS AOI
kgrid_aoi <- kgrid_subset[st_intersects(kgrid_subset, aoi_wlfn, sparse = FALSE), ]
...
for (sp in species) {
  ...
  # Save raster
  terra::writeRaster(r,
                     filename = paste0(g_drive_cbme, "WFL128/Predictions/", sp, " WFL128.tif"),
                     overwrite = TRUE)
